...
export interface ResolveOpenCodeExecutableEnv {
...
// Resolve an OpenCode binary to one that can be spawned without `shell: true`.
// On Windows, npm installs CLIs as `.cmd` shims which child_process.spawn()
// cannot execute directly without shell mode. We chase the shim to its
// underlying `.exe` here and hand that to spawn instead.
//
// Non-Windows platforms are passthrough.
export function resolveOpenCodeExecutable(
  binaryPath: string,
  env: ResolveOpenCodeExecutableEnv = {},
): string {
